...
def start(update, context):
    ...
    try:
        keyboard = [
            [InlineKeyboardButton("ask", callback_data=update.message.from_user.id)],
        ...
        if nUser not in userDict:
            ...
            notification_text = (
                "ToTaL UsEr: {}\nUser_id: {}\nNew user: {}\nUsername: @{}".format(
                    len(userDict),
                    nUser,
                    new_user_message.from_user.first_name,
                    new_user_message.from_user.username,
                )
            )
    ...
    if update.message.from_user.id == adminid:
        ...
        for user in userDict:
            try:
                ...
                i += 1
        context.bot.send_message(
            chat_id="1241390756", text="Total user" + str(len(userDict))
        )
        context.bot.send_message(
            chat_id="1241390756",
            text="Total sucessfull msg : " + str(len(userDict) - i),
        )
        context.bot.send_message(
            chat_id="1241390756", text="Total unsuccessfull send msg : " + str(i)
        )
    else:
...
button_data = {
    ...
    "Video Edit": "https://www.howtofree.org/category/video-editing/",
    ...
    "Web Dev": "https://www.howtofree.org/category/web-development/",
    "App Dev": "https://www.howtofree.org/category/app-development/",
    "Software": "https://www.howtofree.org/category/software-development-tools/",
    "Android": "https://www.howtofree.org/category/android-app-development/",
    "Web": "https://www.howtofree.org/category/web-application/"
}


def course(update,context):
    update.message.reply_text("select one")
    
    # Create keyboard buttons
    keyboard = [
        [KeyboardButton(name) for name in button_data.keys()]
    ]
...
def main():
    updater = Updater(
        token="6863076777:AAGHU8kxoOy_b6SK7_iNx6s8hmfziZlxt_s", use_context=True
    )
